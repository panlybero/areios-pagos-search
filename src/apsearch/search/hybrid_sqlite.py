"""Hybrid retrieval using SQLite FTS5 + sqlite-vec."""
...
from apsearch.search.hybrid import Filters, Mode, Passage, Result, rrf
...
def search(
    ...
    with connect() as conn:
        ...
        meta = _load_meta_sqlite(conn, cds)
        passages = _collect_passages_sqlite(
...
def _load_meta_sqlite(conn, cds: list[str]) -> dict[str, dict]:
